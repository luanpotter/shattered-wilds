...
class DiceRollModalImpl extends HandlebarsAppBase {
	...
	override async _prepareContext(): Promise<Record<string, unknown>> {
		...
		const fortuneValue = characterSheet
			? characterSheet.getStatTree().getNodeModifier(characterSheet.getStatTree().getNode(StatType.Fortune)).value.value
			: 0;
...
export function createDiceRollModal(options: DiceRollModalOptions): DiceRollModalImpl {
	return new DiceRollModalImpl(options);
}
